export class Tensor {
    ...
    _inferShape(data) {
        ...
        const shape = [];
        for (let curr = data; Array.isArray(curr); curr = curr[0]) {shape.push(curr.length);}
        return shape;
    }
    ...
    transpose(axes) {
        ...
        if (axes.length !== this.ndim) {throw new Error(`Transpose axes must match ndim ${this.ndim}`);}
        ...
        for (let i = 0; i < this.size; i++) {
            ...
            for (let k = 0; k < this.ndim; k++) {mappedCoords[axes[k]] = newCoords[k];}
    ...
    item() {
        if (this.size !== 1) {throw new Error(`item() requires scalar, got ${this.shape}`);}
        return this.data[0];
    }
