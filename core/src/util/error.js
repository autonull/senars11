...
export function logDetailedError(error, metadata = {}, component = '') {
    try {
        ...
        Logger.error(`${prefix}${errorObj.message}`, { component, timestamp: new Date().toISOString(), ...metadata, errorType: errorObj?.constructor?.name });
    } catch {
...
export class ErrorHandler {
    #h;
    constructor(componentName = '') { this.#h = createErrorHandler(componentName); }
    logError(error, context = {}, level = 'error') { return this.#h.logError(error, context, level); }
    wrapError(error, message, context = {}) { return this.#h.wrapError(error, message, context); }
    async safeAsync(asyncFn, context = '', contextInfo = {}, defaultValue = null) { return this.#h.safeAsync(asyncFn, context, contextInfo, defaultValue); }
    safeSync(syncFn, context = '', contextInfo = {}, defaultValue = null) { return this.#h.safeSync(syncFn, context, contextInfo, defaultValue); }
    async executeWithHandling(operation, context = '', options = {}) { return this.#h.executeWithHandling(operation, context, options); }
    executeSyncWithHandling(operation, context = '', options = {}) { return this.#h.executeSyncWithHandling(operation, context, options); }
}
