...
export class NAR extends BaseComponent {
    ...
    async _processNewTask(task, source, originalInput, parsed, options = {}) {
        ...
        if (wasAdded) {
            try {
                await this._eventBus.emit(IntrospectionEvents.TASK_INPUT,
                    {task, source, originalInput, parsed}, {traceId: options.traceId});
                await this._eventBus.emit(IntrospectionEvents.TASK_ADDED, {task}, {traceId: options.traceId});
                if (this._focus && this._focus.addTaskToFocus(task)) {
                    await this._eventBus.emit(IntrospectionEvents.TASK_FOCUS, task, {traceId: options.traceId});
                }
            } catch (error) {
    ...
    async step(options = {}) {
        try {
            ...
            await this._eventBus.emit('streamReasoner.step', {
                results,
                count: results.length
            }, {traceId: options.traceId});
            return results;
        } catch (error) {
            await this._eventBus.emit('streamReasoner.error', {error: error.message}, {traceId: options.traceId});
