...
export class NarsGPTStrategy extends PremiseFormationStrategy {
    constructor(config = {}) {
        ...
        // Legacy compat
        if (config.perspectiveSwapEnabled === false) {
            this.perspectiveMode = 'none';
        }
        if (config.perspectiveSwapEnabled === true && !config.perspectiveMode) {
            this.perspectiveMode = 'swap';
        }
    ...
    async _getRelevantItems(queryText, memory) {
        ...
        try {
            ...
            for (const [, concept] of concepts) {
                ...
                if (tasks.length >= this.relevantViewSize * 2) {
                    break;
                }
            }
            ...
            for (const task of tasks) {
                ...
                if (results.length >= this.relevantViewSize) {
                    break;
                }
            }
    ...
    async batchScore(queryText, candidateTexts) {
        if (!this.embeddingLayer || !candidateTexts?.length) {
            return [];
        }
        try {
            const queryEmbed = await this.embeddingLayer.getEmbedding(queryText);
    ...
    async checkGrounding(input) {
        ...
        if (!this.embeddingLayer || !this.groundings.size) {
    ...
    eternalize(memory, currentTime) {
        ...
        if (count) {
            this._emit('narsgpt:eternalized', {count});
        }
    }
    ...
    perspectiveSwap(text) {
        if (this.perspectiveMode === 'none' || !text) {
            return text;
        }
        if (this.perspectiveMode === 'neutralize') {
            return this.perspectiveNeutralize(text);
        }

        this._metrics.perspectiveOps++;
        let result = ` ${text} `;
        for (const [pattern, placeholder] of SWAP_PATTERNS) {
