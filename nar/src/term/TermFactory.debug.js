...
export class TermFactory extends BaseComponent {
    ...
    _createCompound(operator, components) { console.error("DEBUG _createCompound:", { operator, compNames: components.map(c=>c?.name), compTypes: components.map(c=>c?.constructor?.name) });
    ...
    _cacheLookupOrCreate(operator, components) { console.error("DEBUG _cacheLookupOrCreate:", { operator, compNames: components.map(c=>c?.name) });
    ...
    delta(term) { console.error('DEBUG delta called, term:', term?.name, 'is Term:', term?.constructor?.name); const r = this._createCompound('Δ', [term]); console.error('DEBUG delta returning:', r?.name, r?._type, r?._operator); return r; }
    ...
    _createAndCache(operator, components, name) { console.error("DEBUG _createAndCache:", { operator, name, compNames: components.map(c=>c?.name) });
