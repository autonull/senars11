...
/**
 * Proper Hybrid Demo: NL ↔ Narsese interaction through LM Rules
...
async function createProperHybridDemo() {
    console.log("🚀 Starting Proper Hybrid Demo: NL ↔ Narsese via LM Rules\n");
    ...
    console.log("🎯 SETTING UP LM RULES FOR HYBRID REASONING:\n");

    // Create an LM rule for hypothesis generation that gets triggered during reasoning
    console.log("📝 Creating LM Hypothesis Generation Rule");
    ...
    try {
        ...
        if (nar.reasoner && typeof nar.reasoner.addRule === 'function') {
            ...
            console.log("   ✅ LM Hypothesis Generation Rule added to reasoner\n");
        } else {
            console.log("   ⚠️  Could not add LM rule (reasoner interface may differ)\n");
        }
    } catch (error) {
    ...
    console.log("📝 Creating LM Concept Elaboration Rule");
    ...
    try {
        ...
        if (nar.reasoner && typeof nar.reasoner.addRule === 'function') {
            ...
            console.log("   ✅ LM Concept Elaboration Rule added to reasoner\n");
        } else {
            console.log("   ⚠️  Could not add elaboration rule (reasoner interface may differ)\n");
        }
    } catch (error) {
