...
export class ActionModule extends CognitiveModule {
    ...
    async process(input, context = {}) {
        ...
        if (this.actionHistory.length > this.config.actionHistoryLimit) {
    ...
    async getActionValues(state) {
        return Array(this.config.actionSpace?.n ?? 4).fill(0);
    }

    randomAction() {
        return Math.floor(Math.random() * (this.config.actionSpace?.n ?? 4));
    }
