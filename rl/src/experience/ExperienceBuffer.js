...
import {mergeConfig, SumTree} from '../utils/index.js';
...
export class ExperienceBuffer extends Component {
    ...
    async store(experience) {
        ...
        if (this.count >= this.capacity) {
            ...
            if (this.sumTree) {
                this.sumTree.update(this.pos, 1.0);
            }
        } else {
            this.experiences.push(exp);
            if (this.sumTree) {
                this.sumTree.add(1.0);
            }
            this.count++;
        }
    ...
    async _updateCausalGraph(experience) {
        const {state, action, nextState, reward} = experience;
        await this.causalReasoner?.learn(JSON.stringify(state), JSON.stringify(nextState), JSON.stringify({
            action,
            reward
        }));
