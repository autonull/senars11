...
export class HarnessOptimizer {
  constructor(config, modelRouter, auditSpace) {
    ...
    Logger.info('[HarnessOptimizer] Initialized', { evalInterval: this.evalInterval, minScoreImprovement: this.minScoreImprovement });
  }
  ...
  async runOptimizationCycle() {
    ...
    try {
      ...
      if (!failures.length) return { applied: false, delta: null, reason: 'No failures to analyze' };

      const currentPrompt = this._readCurrentPrompt();
      if (!currentPrompt) return { applied: false, delta: null, reason: 'No current prompt found' };

      const proposal = await this._proposeChange(failures, currentPrompt);
      if (!proposal?.diff) return { applied: false, delta: null, reason: 'No change proposed' };
  ...
  async _scanTraceFiles() {
    ...
    if (!existsSync(todayDir)) return failures;
...
Return ONLY a unified diff (diff -u format) showing the change. Do not explain.`;
    ...
    try {
      ...
      return { diff: diffMatch ? (diffMatch[1] || diffMatch[0]) : result.response, model: result.model, latency: result.latency };
    } catch (error) {
    ...
    try {
      ...
      for (const line of diffLines) {
        if (line.startsWith('-')) {
          ...
          if (idx !== -1) { pendingDelete = idx; result.splice(idx, 1); }
        } else if (line.startsWith('+')) {
          const content = line.slice(1);
          if (pendingDelete !== -1) { result.splice(pendingDelete, 0, content); pendingDelete = -1; }
  ...
  async _applyCandidate(score) {
    try {
      ...
      try {
        ...
        execSync(`git commit -m "harness-update: cycle ${this.cycleCount}, score=${score.toFixed(3)}"`, { cwd: process.cwd(), stdio: 'pipe' });
        Logger.info('[HarnessOptimizer] Committed harness update');
      } catch (gitError) {
        Logger.warn('[HarnessOptimizer] Git commit failed:', gitError.message);
      }

      if (this.auditSpace) await this.auditSpace.emitHarnessModified(this.cycleCount, score);
    } catch (error) {
  ...
  async generateDiff() {
    if (!existsSync(this.candidatePath)) return null;
    if (!existsSync(this.harnessPath)) return readFileSync(this.candidatePath, 'utf-8');
    try {
      return execSync(`git diff --no-index ${this.harnessPath} ${this.candidatePath}`, { cwd: process.cwd(), encoding: 'utf-8' });
    } catch (error) {
