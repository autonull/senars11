...
export class IntrospectionOps {
  ...
  generateManifest() {
    if (!this.config.capabilities?.runtimeIntrospection) return '(manifest :restricted true)';

    const currentCycle = this.loopState?.cycleCount ?? 0;
    if (this.manifestCache && currentCycle < this.manifestCacheExpiry) return this.manifestCache;
  ...
  listSkills() {
    if (!this.config.capabilities?.runtimeIntrospection) return '(skill-inventory :restricted true)';
  ...
  getState(key) {
    if (!this.loopState) return `(agent-state :error "loop-state-not-available")`;
    ...
    switch (key) {
      ...
      case '&model-override': case 'model-override':
  ...
  _getCapabilityState() {
    ...
    const result = {};
    for (const [key, value] of Object.entries(caps)) result[key] = Boolean(value);
    return result;
  }
  ...
  _objToSExpr(obj) {
    ...
    return `(${Object.entries(obj).map(([key, value]) => {
      ...
      else if (typeof value === 'object') formattedValue = this._objToSExpr(value);
