import {fallbackAgentDir, generateId, Logger, resolveWithFallback} from '@senars/core';
...
export class MeTTaSkillRegistrar {
    ...
    async registerAll() {
        ...
        this.dispatcher.register('think', async content => {
        ...
        this.dispatcher.register('cognitive-cycle', async stim => {
            try {
                ...
                const stimulus = typeof stim === 'string' ? {
                    type: 'message',
                    content: stim,
                    source: 'agent-loop'
                } : stim;
        ...
        this.dispatcher.register('attend', async (content, priority) => {
            ...
            this.loopState.wm.push({
                content: String(content),
                priority: pri,
                ttl,
                cycleAdded: this.loopState.cycleCount
            });
        ...
        this.dispatcher.register('send', async content => {
            ...
            if (msg === this.loopState.lastsend) {
        ...
        this.dispatcher.register('spawn-agent', async (task, cycleBudget) => {
            if (!this.agent.virtualEmbodiment) {
        ...
        if (this.cap('webSearchSkill')) {
            this.dispatcher.register('search', async query => {
                if (this.agent.toolInstances?.websearch?.search) {
                    return JSON.stringify(await this.agent.toolInstances.websearch.search(String(query))).slice(0, 2000);
                }
        ...
        if (this.cap('fileReadSkill')) {
            this.dispatcher.register('read-file', async path => {
                if (this.agent.toolInstances?.file?.readFile) {
                    return await this.agent.toolInstances.file.readFile(String(path));
                }
        ...
        if (this.cap('fileWriteSkill')) {
            this.dispatcher.register('write-file', async (path, content) => {
                if (this.agent.toolInstances?.file?.writeFile) {
                    await this.agent.toolInstances.file.writeFile(String(path), String(content));
                    return `written: ${path}`;
                }
                return '(file tool not configured)';
            }, 'fileWriteSkill', ':local-write');
            this.dispatcher.register('append-file', async (path, content) => {
                if (this.agent.toolInstances?.file?.appendFile) {
                    await this.agent.toolInstances.file.appendFile(String(path), String(content));
        ...
        if (this.cap('semanticMemory')) {
            await this.#registerSemanticMemorySkills();
        }
        if (this.cap('goalPursuit')) {
            this.#registerGoalPursuitSkills();
        }
        if (this.cap('persistentHistory')) {
            this.#registerPersistentHistorySkills();
        }
        if (this.cap('memorySnapshots')) {
            this.#registerMemorySnapshotSkills();
        }
        if (this.cap('actionTrace')) {
            this.#registerActionTraceSkills();
        }
        if (this.cap('coordinatorMode')) {
            this.#registerCoordinatorSkills();
        }
        if (this.cap('separateEvaluator')) {
            this.#registerEvaluatorSkills();
        }
        if (this.cap('memoryConsolidation')) {
            this.#registerConsolidationSkills();
        }
        if (this.cap('selfModifyingSkills')) {
            this.#registerSelfModifyingSkills();
        }
        if (this.cap('multiModelRouting')) {
            await this.#registerMultiModelSkills();
        }
    }
    ...
    async #registerSemanticMemorySkills() {
        ...
        this.dispatcher.register('remember', async (content, type, tags) => {
            const id = await this.agent._semanticMemory.remember({
                content: String(content),
                type: type ?? 'episodic',
                tags: tags ?? [],
                source: 'agent-loop'
    ...
    #registerEvaluatorSkills() {
        this.dispatcher.register('nar-revision', async (term, evidence) => {
            ...
            const beliefs = this.agent.nar.memory?.getBeliefsByTerm?.(String(term)) ?? [];
            if (beliefs.length === 0) {
                return `(revision-error :reason "no-beliefs-for-term" :term "${term}")`;
            }
