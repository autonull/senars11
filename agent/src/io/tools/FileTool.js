...
export class FileTool {
    constructor(config = {}) {
        ...
        if (!fs.existsSync(this.workspace)) {
            try { fs.mkdirSync(this.workspace, { recursive: true }); }
            catch (e) { Logger.error('Failed to create workspace:', e); }
        }
    }
    ...
    _resolvePath(filePath) {
        ...
        if (!fullPath.startsWith(this.workspace)) throw new Error('Access denied: Path is outside workspace');
        return fullPath;
    }
