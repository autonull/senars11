...
import {SkillDispatcher} from '../skills/index.js';
...
export class IntelligentMessageProcessor {
    ...
    async _initSkillsAndAudit() {
        ...
        if (isEnabled(agentCfg, 'auditLog')) {
            ...
            try {
                ...
                Logger.warn('[IMP] AuditSpace init failed:', e.message);
            }
        }
        ...
        if (isEnabled(agentCfg, 'sExprSkillDispatch')) {
            ...
            try {
                ...
                Logger.warn('[IMP] SkillDispatcher init failed:', e.message);
            }
    ...
    _registerDefaultSkillHandlers() {
        ...
        this._skillDispatcher.register('metta', async (expr) => {
            try {
                return {result: this.agent.metta?.run(expr)};
            } catch (e) {
    ...
    async processMessage(msg) {
        ...
        if (!this._shouldRespond(classification, isMentioned, isPrivate)) {
            if (this.config.learnFromConversation) {
                await this._learnFromMessage(msg);
            }
        ...
        try {
            ...
            if (response) {
                ...
                if (this.config.learnFromConversation) {
                    await this._learnFromExchange(msg, response);
                }
                await this._auditEmit('response-sent', {
                    to: from,
                    channel,
                    content: String(response).substring(0, 500)
                });
            }
    ...
    async _startupOrient(contextKey) {
        ...
        try {
            ...
            if (beliefs.length) {
                context.startupBeliefs = beliefs.slice(0, 5).map(b => String(b));
            }
        } catch (e) {
            Logger.debug('[IMP] Startup orient beliefs failed:', e.message);
        }
        ...
        if (this._auditSpace) {
            try {
                ...
                if (recentErrors.length) {
                    context.startupFeedback = recentErrors.map(e => e.error?.substring(0, 200)).join('; ');
                }
            } catch (e) {
                Logger.debug('[IMP] Startup orient feedback failed:', e.message);
            }
        }
        ...
        if (this.agent.metta) {
            try {
                ...
                Logger.debug('[IMP] Startup orient history failed:', e.message);
            }
        }
    }
    ...
    async _buildSlotBeliefs(parts, content) {
        try {
            ...
            if (relevant.length) {
                parts.BELIEFS = relevant.map(b => String(b)).join('\n');
            }
        } catch (e) {
            Logger.debug('NARS beliefs query failed:', e.message);
        }
    }
    ...
    async _buildSlotFeedback(parts) {
        ...
        try {
            ...
            Logger.debug('AuditSpace feedback failed:', e.message);
        }
    }
    ...
    _buildSlotSkills(parts) {
        ...
        if (skillDefs && !skillDefs.startsWith('(no skills')) {
            parts.SKILLS = skillDefs;
        }
    }

    _buildSlotStartupOrient(parts, context) {
        const segments = [];
        if (context.startupBeliefs?.length) {
            segments.push(`[BELIEFS] ${context.startupBeliefs.join('; ')}`);
        }
        if (context.startupFeedback) {
            segments.push(`[FEEDBACK] ${context.startupFeedback}`);
        }
        if (context.startupHistory) {
            segments.push(`[HISTORY] ${context.startupHistory}`);
        }
        if (segments.length) {
            parts.STARTUP_ORIENT = segments.join('\n');
        }
    }
    ...
    _buildSystemPrompt(useSkillDispatch = false) {
        ...
        if (!useSkillDispatch || !this._skillDispatcher) {
            return base;
        }
        const skillDefs = this._skillDispatcher.getActiveSkillDefs();
    ...
    async _invokeLLM(contextStr, inputContent) {
        ...
        if (!cmds?.length || error) {
            return {response: text, skillResults: null};
        }
    ...
    async _handleCommand(content, from, channel, context) {
        ...
        const builtInCommands = {
            ...
            remember: async () => {
                ...
                if (!text) {
                    return 'Remember what? Usage: !remember <text>';
                }
            ...
            think: async () => {
                ...
                if (!text) {
                    return 'Think about what? Usage: !think <topic>';
                }
                if (context.wmEntries) {
                    context.wmEntries.push({content: text, priority: 0.7, ttl: 5});
                }
            ...
            attend: async () => {
                ...
                if (!text) {
                    return 'Attend to what? Usage: !attend <topic>';
                }
                if (context.wmEntries) {
                    context.wmEntries.push({content: text, priority: 0.9, ttl: 3});
                }
        ...
        if (handler) {
            try {
                return {response: await handler(), action: 'command', skillResults: null};
            } catch (error) {
                return {response: `Error: ${error.message}`, action: 'command', skillResults: null};
            }
        }
        ...
        if (this.agent.commandRegistry?.get(cmd)) {
            try {
                ...
                return {response: `Command error: ${error.message}`, action: 'command', skillResults: null};
            }
        }
    ...
    async _dumpContextDump(channel, from, context) {
        ...
        if (ctx.startupBeliefs || ctx.startupFeedback || ctx.startupHistory) {
            lines.push('\nSTARTUP_ORIENT:');
            if (ctx.startupBeliefs) {
                lines.push(`  BELIEFS: ${ctx.startupBeliefs.join('; ')}`);
            }
            if (ctx.startupFeedback) {
                lines.push(`  FEEDBACK: ${ctx.startupFeedback}`);
            }
            if (ctx.startupHistory) {
                lines.push(`  HISTORY: ${ctx.startupHistory}`);
            }
        }
    ...
    async _handleQuestion(content, context) {
        ...
        return {
            response: result?.response ?? "Not sure about that one.",
            action: 'answer',
            skillResults: result?.skillResults ?? null
        };
    }
    ...
    _saveEvictedToMeTTa(evicted, context) {
        ...
        for (const msg of evicted) {
            if (msg.from === this.config.botNick) {
                ...
                if (prevMsg) {
                    this._storeMeTTaAtom(`(conversation "evicted" "${prevMsg.from}" "${this._safeAtom(prevMsg.content)}" "${this._safeAtom(msg.content)}")`);
                }
            }
        }
    }
    ...
    _heuristicClassify(content) {
        ...
        if (lower.endsWith('?') || QUESTION_STARTERS.has(lower.split(' ')[0])) {
            return {type: 'question', confidence: 0.8};
        }
        if (GREETING_KEYWORDS.has(lower.split(' ')[0])) {
            return {type: 'greeting', confidence: 0.85};
        }
        if ([...FAREWELL_KEYWORDS].some(f => lower.includes(f))) {
    ...
    _storeMeTTaAtom(atom) {
        try {
            ...
            Logger.debug('[IMP] MeTTa atom store failed:', e.message);
        }
    }
    ...
    async _auditEmit(type, data) {
        if (this._auditSpace) {
            try {
                await this._auditSpace.emit(type, data);
            } catch (e) {
                Logger.debug('[IMP] Audit emit failed:', e.message);
            }
        }
    }
    ...
    async _auditEmitLlmCall(model, promptChars, responseChars, latencyMs, tokenUsage) {
        if (this._auditSpace) {
            try {
                await this._auditSpace.emitLlmCall(model, promptChars, responseChars, latencyMs, tokenUsage);
            } catch (e) {
                Logger.debug('[IMP] LLM audit emit failed:', e.message);
            }
        }
    }
    ...
    getStats() {
        return {...this.stats, activeContexts: this.contexts.size, cacheSize: this.classificationCache.size};
    }
