...
export class WorkingMemory {
    ...
    getContents() {
        return this.#slots.map(s => ({content: s.content, activation: s.activation}));
    }
...
export class EpisodicMemory {
    ...
    add(episode) {
        this.#episodes.push({...episode, id: generateId('ep')});
    ...
    getRelevant(current, count = 5) {
        ...
        return this.#episodes.map(ep => ({
            episode: ep,
            score: this.#relevanceScore(ep, currentContent)
        })).sort((a, b) => b.score - a.score).slice(0, count).map(s => s.episode);
    }
...
export class CognitiveSemanticMemory {
    ...
    add(knowledge) {
        const key = this.#indexKey(knowledge);
    ...
    #prune() {
        const sorted = [...this.#knowledge.entries()].map(([key, items]) => ({
            key,
        ...
        for (const entry of sorted) {
            if (removed >= this.#knowledge.size - this.limit) {
                break;
            }
            entry.items.pop();
            removed++;
...
export class ProceduralMemory {
    ...
    add(rule) {
        this.#rules.set(generateId('rule'), {...rule, usage: 0});
    }
    ...
    match(perception) {
        ...
        for (const rule of this.#rules.values()) {
            ...
            if (condition.entity && features.entities?.some(e => e.value === condition.entity)) {
                score += 0.3;
            }
            if (score > 0.4) {
...
export class AttentionMechanism {
    ...
    select(perception, workingMemory, semanticMemory) {
        ...
        workingMemory.getContents().forEach(wm => {
            if (this.#contentOverlap(perception.content, wm.content)) {
