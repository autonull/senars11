...
export function detectSentiment(text) {
    ...
    const lower = text.toLowerCase();
    if (SENTIMENT_WORDS.positive.some(w => lower.includes(w))) return 'positive';
    if (SENTIMENT_WORDS.negative.some(w => lower.includes(w))) return 'negative';
    return 'neutral';
}
...
export function detectResponseType(text) {
    ...
    for (const { regex, type } of RESPONSE_PATTERNS) {
        if (lower.match(regex)) return type;
    }
    return 'response';
}
...
export function extractFact(text) {
    ...
    for (const pattern of FACT_PATTERNS) {
        ...
        if (match) return { type: 'fact', content: match[0], confidence: 0.7 };
    }
    return null;
}
